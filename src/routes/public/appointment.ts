...
publicAppointmentRouter.post('/public/appointments', authPublicMiddleware, async (req: Request, res: Response) => {
  ...
  console.log('[PUBLIC_APPOINTMENTS] Starting new appointment creation');
    
  const contactChannelId = req.headers['x-sender-channel'] as string;
  const userId = req.headers['x-user-id'] as string;

  console.log(`[PUBLIC_APPOINTMENTS] Received appointment data - ContactId: ${contactChannelId}, ServiceIds: ${serviceIds}, Date: ${date}, Time: ${time}`);

  if (!contactChannelId || !serviceIds || !date || !time || !Array.isArray(serviceIds)) {
    console.log('[PUBLIC_APPOINTMENTS] Error: Missing required fields');
  ...
  if (!contact) {
    console.log(`[PUBLIC_APPOINTMENTS] Error: Contact with channelId ${contactChannelId} not found. Needs start a chat and try again`);
  ...
  for (const serviceId of serviceIds) {
    ...
    if (!service.value) {
      console.log(`[PUBLIC_APPOINTMENTS] Error: Service with id ${serviceId} not found`);
  ...
  if (!isAvailable) {
    console.log('[PUBLIC_APPOINTMENTS] Error: The selected time slot is not available');
  ...
  try {
    ...
    console.log(`[PUBLIC_APPOINTMENTS] Appointment created successfully - Id: ${appointment.id}`);
    res.status(201).json(appointment);
  } catch (error) {
    console.error(`[PUBLIC_APPOINTMENTS] Error creating appointment: ${error}`);
...
publicAppointmentRouter.get('/public/appointments', authPublicMiddleware, async (req: Request, res: Response) => {
  console.log('[PUBLIC_APPOINTMENTS] Fetching contact appointments');
  ...
  console.log(`[PUBLIC_APPOINTMENTS] Total appointments fetched: ${appointments.length}`);
  res.json({
...
publicAppointmentRouter.put('/public/appointments/:id', authPublicMiddleware, async (req: Request, res: Response) => {
  console.log('[PUBLIC_APPOINTMENTS] Starting appointment update');
  
  const { id } = req.params;
  const { serviceIds, date, time } = req.body;
  console.log(`[PUBLIC_APPOINTMENTS] Received appointment data - Id: ${id}, ServiceIds: ${serviceIds}, Date: ${date}, Time: ${time}`);
  ...
  if (!appointment.value) {
    console.log(`[PUBLIC_APPOINTMENTS] Error: Appointment with id ${id} not found`);
  ...
  try {
    ...
    console.log(`[PUBLIC_APPOINTMENTS] Appointment updated successfully - Id: ${updatedAppointment.id}`);
    res.json(updatedAppointment);
  } catch (error) {
    console.error(`[PUBLIC_APPOINTMENTS] Error updating appointment: ${error}`);
...
publicAppointmentRouter.delete('/public/appointments/:id', authPublicMiddleware, async (req: Request, res: Response) => {
  console.log('[PUBLIC_APPOINTMENTS] Starting appointment deletion');
  ...
  try {
    ...
    console.log(`[PUBLIC_APPOINTMENTS] Appointment deleted successfully - Id: ${appointment.value.id}`);
    res.status(204).end();
  } catch (error) {
    console.error(`[PUBLIC_APPOINTMENTS] Error deleting appointment: ${error}`);
...
publicAppointmentRouter.get('/public/appointments/check', async (req: Request, res: Response) => {
  console.log('[PUBLIC_APPOINTMENTS] Checking appointment availability');
  
  const { date, time, duration } = req.query;

  if (!date || !time || !duration) {
    console.log('[PUBLIC_APPOINTMENTS] Error: Date and time are required');
  ...
  if (isAvailable) {
    console.log('[PUBLIC_APPOINTMENTS] The selected time slot is available');
    res.json({ available: true });
  } else {
    console.log('[PUBLIC_APPOINTMENTS] The selected time slot is not available');
